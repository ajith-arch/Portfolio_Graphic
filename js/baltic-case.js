/**
 * Baltic × Spaceone case study — floating dock nav, scroll spy, reveal.
 */
(function (global) {
  'use strict';

  var LIVE_URL = 'https://ajith-arch.github.io/baltic/';

  function initBalticReveal() {
    var els = document.querySelectorAll('.bl-reveal');
  ...
  function initBalticDock() {
    var dock = document.getElementById('blDock');
    if (!dock) return;

    var tabs = dock.querySelectorAll('.bl-dock__tab[data-section]');
    var sections = [];
  ...
  function initBalticCase() {
    initBalticReveal();
    initBalticDock();
  }

  global.initBalticCase = initBalticCase;
  global.BALTIC_LIVE_URL = LIVE_URL;
