/**
 * Campus Store case study — floating dock nav, scroll spy, reveal.
 */
(function (global) {
  'use strict';

  var LIVE_URL = 'https://ajith-arch.github.io/campusstore/';

  function initCampusStoreReveal() {
  ...
  function initCampusStoreDock() {
    var dock = document.getElementById('csDock');
    if (!dock) return;

    var tabs = dock.querySelectorAll('.cs-dock__tab[data-section]');
    var sections = [];
  ...
  function initCampusStoreWalkthrough() {
    var wrap = document.querySelector('.cs-walkthrough__video-wrap');
    var video = wrap && wrap.querySelector('.cs-walkthrough__video');
  ...
  function initCampusStoreCase() {
    initCampusStoreReveal();
    initCampusStoreDock();
    initCampusStoreWalkthrough();
  }

  global.initCampusStoreCase = initCampusStoreCase;
  global.CAMPUS_STORE_LIVE_URL = LIVE_URL;
